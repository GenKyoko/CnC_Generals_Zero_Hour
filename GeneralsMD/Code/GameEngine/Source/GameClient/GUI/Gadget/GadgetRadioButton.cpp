												...
												GameWindow *except )
{
	...
	//
	if( window != except && BitTestEA( window->winGetStyle(), GWS_RADIO_BUTTON ) )
	{
														 ...
														 WindowMsgData mData1, WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_LEFT_UP:
		{
			...
			if( BitTestEA( instData->getState(), WIN_STATE_SELECTED ) == FALSE )
			{
				RadioButtonData *radioData = (RadioButtonData *)window->winGetUserData();
			...
			else if( BitTestEA( instData->getState(), WIN_STATE_HILITED ) == FALSE )
			{
