...
public:
	...
	*/
	...
	WWINLINE void Matrix4x4::Init_PerspectiveProjection
	(
		float left,
		float right,
		float bottom,
...
WWINLINE void Matrix4x4::Init_Perspective(float fovY, float aspect, float znear, float zfar, float not_used)
{
	assert(znear > 0.0f);
	assert(zfar > 0.0f);
	assert(aspect > 0.0f);
