...
void MapObjectProps::_TeamToDict(void)
{
  ...
  if ( pDoc )
  {
    ...
    DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, newDict.getNthKey(0), m_allSelectedDicts.size(), pDoc, true);
...
void MapObjectProps::_NameToDict(void)
{
  ...
  if ( pDoc )
  {
    ...
    DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, newDict.getNthKey(0), m_allSelectedDicts.size());
    pDoc->AddAndDoUndoable(pUndo);
    REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
    // Update is called by Do																																																						
  }
...
void MapObjectProps::_PrebuiltUpgradesToDict(void)
{
  ...
  if ( pDoc != NULL )
  {
    DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, NAMEKEY_INVALID, m_allSelectedDicts.size(), pDoc, true);
    pDoc->AddAndDoUndoable(pUndo);
    REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
  }
}
...
void MapObjectProps::_VisibilityToDict(void)
{
  ...
  if ( pDoc != NULL )
  {
	  ...
	  DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, TheKey_objectVisualRange, m_allSelectedDicts.size());
	  pDoc->AddAndDoUndoable(pUndo);
	  REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
	  // Update is called by Do
  }
...
void MapObjectProps::_ShroudClearingDistanceToDict(void)
{
  ...
  if ( pDoc != NULL )
  {
	  ...
	  DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, TheKey_objectShroudClearingDistance, m_allSelectedDicts.size());
	  pDoc->AddAndDoUndoable(pUndo);
	  REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
	  // Update is called by Do
  }
...
void MapObjectProps::clearCustomizeFlag( CWorldBuilderDoc* pDoc, MultipleUndoable * ownerUndoable )
{
  ...
  DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientCustomized, m_allSelectedDicts.size(), pDoc, true);
  ...
  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientMaxRange, m_allSelectedDicts.size(), pDoc, false);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.
  
  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientMinVolume, m_allSelectedDicts.size(), pDoc, false);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.
  
  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientVolume, m_allSelectedDicts.size(), pDoc, false);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.
  
  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientMinRange, m_allSelectedDicts.size(), pDoc, false);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.

  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientLooping, m_allSelectedDicts.size(), pDoc, false);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.

  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientLoopCount, m_allSelectedDicts.size(), pDoc, false);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.

  pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, empty, TheKey_objectSoundAmbientPriority, m_allSelectedDicts.size(), pDoc, true);
  ownerUndoable->addUndoable(pUndo);
  REF_PTR_RELEASE(pUndo); // belongs to ownerUndoable now.
}
...
void MapObjectProps::attachedSoundToDict(void)
{
  ...
  if ( pDoc != NULL )
  {
    ...
    DictItemUndoable *pDictUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, TheKey_objectSoundAmbient, m_allSelectedDicts.size(), pDoc, true);
...
void MapObjectProps::customizeToDict(void)
{
  ...
  if ( pDoc != NULL )
  {
    ...
    if ( customizeCheckbox->GetCheck() == 1 )
    {
      ...
      DictItemUndoable *pUndo = new DictItemUndoable(m_allSelectedDicts.begin()._Ptr, newDict, TheKey_objectSoundAmbientCustomized, m_allSelectedDicts.size(), pDoc, true);
      pDoc->AddAndDoUndoable(pUndo);
      REF_PTR_RELEASE(pUndo); // belongs to pDoc now.
      // Update is called by Doc
    }
